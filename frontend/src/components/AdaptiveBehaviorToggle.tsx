interface AdaptiveBehaviorToggleProps {
  enabled: boolean;
...
const DEFAULT_DESCRIPTION =
  "Use the engagement signals above to adjust the agent during the interview (e.g. offer a break, soften a question, change speaking pace). Starts in shadow mode, which logs intended actions without applying them. Requires engagement tracking. Off by default.";

export default function AdaptiveBehaviorToggle({
  enabled,
  onChange,
  disabled = false,
  disabledReason,
  description = DEFAULT_DESCRIPTION,
}: AdaptiveBehaviorToggleProps) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            role="switch"
            aria-checked={isOn}
            aria-label="Enable adaptive behavior"
            disabled={disabled}
